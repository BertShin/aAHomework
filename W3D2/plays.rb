...
class Play
  ...
  def self.find_by_title(title)
    find_title = PlayDBConection.instance.execute(<<-SQL, title)
      SELECT
        *
      FROM
        plays
      WHERE
        title = ?
    SQL
    return nil unless find_title.length > 0

    Play.new(find_title.first)
  end
...
class Playwright
  ...
  def update
    raise "#{self} isn't in the database!" unless @id
  ...
  def get_plays
    raise "#{self} not in database!" unless @id
    play = PlayDBConnection.instance.execute(<<-SQL, @id)
      SELECT
        *
      FROM
        plays
